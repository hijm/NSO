...
"""
CORREGIDO POR HEINE
Func nao pedia parametro - criado parametro device_name
chamada de root.devices.device, nao esperava o [device_name]
Interface vlan nao existe no IOS, mudado para Vlan
"""
def create_list_item(device_name):
    """
    ...
    with ncs.maapi.single_write_trans('admin', 'python', groups=['ncsadmin']) as t:
        root = ncs.maagic.get_root(t)
        root.devices.device[device_name].config.interface.Vlan.create("200")
        t.apply()
create_list_item("router0")

def create_interface_loopback(device_name, interface_num):
    with ncs.maapi.single_write_trans('admin', 'python',groups=['ncsadmin'])  as t:
        root = ncs.maagic.get_root(t)
        device = root.devices.device[device_name]
        device.config.interface.Loopback.create([interface_num])
        sync_output = device.sync_from.request()
        t.apply()
        #check_commit = configuration.commit.list
    
create_interface_loopback("router1",100)
...
def chec_commit():
    with ncs.maapi.single_write_trans('admin', 'python', ['ncsadmin']) as t:
        root = ncs.maagic.get_root(t)
        #device = root.devices.device[device_name]
        #print("Sync-from: " + device_name)
        #sync_output = device.sync_from.request()
        ncs.maagic.show.configuration.request()
        #print("Sync result: " + str(sync_output.result))
...
#show_commands("running-config", "router1")
...
print("Using leaf lists")
using_leaflists_data("routers")
...
def check_if_interface_exists(device_name, interface_type, interface_number):
    ...
    with ncs.maapi.single_write_trans('admin', 'python', groups=['ncsadmin']) as t:
        root = ncs.maagic.get_root(t)
        device = root.devices.device[device_name]
        print type(device.config.interface[interface_type])
        print device.config.interface[interface_type]

        if interface_number in device.config.interface[interface_type]:
            print("Interface is on the device!")
            print interface_number
        else:
            print("Interface is not on the device!")
check_if_interface_exists("router0", "FastEthernet", "0/0")
...
"""
TEST HEINE 
CORRIGIDO - ORIGINAL NAO TINHA O .config.
PRECISA FAZER SYNC FROM ANTES
"""
def print_interfaces2(device_name,interface_type):
    """
    Prints each interface number on the device of the given type
    """
    with ncs.maapi.single_write_trans('admin', 'python', groups=['ncsadmin']) as t:
        ...
        print (device.config.interface[interface_type])
        for interface in device.config.interface[interface_type]:
            print interface.name
        

print_interfaces2("router0","FastEthernet")